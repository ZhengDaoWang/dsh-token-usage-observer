...
function collectOpencode(dbs) {
	...
	for (const dbPath of candidates) {
		...
		try {
			const rows = db.prepare("SELECT id, time_created, model, tokens_input, tokens_output, tokens_reasoning, tokens_cache_read, tokens_cache_write FROM session").all();
...
function renderSessions(sessions) {
	...
	const lines = ["| 来源 | 会话 | 模型/预设 | 时间 | 请求数 | 输入(未缓存) | 输入(缓存命中) | 缓存写入 | 输出 | 命中率 | 费用 |", "| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |"];

 ...
 * reconciliation; the dashboard view it toggles is a separate React root
...
export interface SidebarEntryOptions {
  /** Full attribute name identifying the injected row (idempotency key), e.g. 'data-dsh-token-usage-entry'. */
  rowAttribute: string
  /** CSS selector matching the injected row, e.g. '[data-dsh-token-usage-entry]'. */
  rowSelector: string
  /** L2 semantic-attribute plugin id; the row outputs data-dsh-plugin="<id>" and data-dsh-part="sidebar-entry". */
  plugin?: string
   ...
   * (its own row included). Keeps the rendered order stable across sibling
   * plugins that self-heal during the same re-render.
   */
...
export function mountSidebarEntry(options: SidebarEntryOptions): () => void {
  ...
  // alive), never mount a second one.
